...
export default {
	...
	formatBootstrap(bcForm) {
		...
		if (($(bcForm).find('fieldset.required').length) > ($(bcForm).find('fieldset.optional').length)) {
			$(bcForm).find('fieldset.optional label').append('<small class="label  label-info">Optional</small>');
		}
		else {
			$(bcForm).find('fieldset.required label').append('<small class="label  label-danger">Required</small>');
		}
	...
	modernizeCaptchas(bcForm) {
		$(bcForm).find('fieldset:contains("module_recaptcha version") span').contents().filter(function() { return this.nodeType == 3 }).each(function(){
		    this.textContent = this.textContent.replace('module_recaptcha version="2"','module_recaptcha version="2" size="compact"');
		});

		$(bcForm).find('fieldset:contains("{module_recaptcha}") span').contents().filter(function() { return this.nodeType == 3 }).each(function(){
		    this.textContent = this.textContent.replace('module_recaptcha','module_recaptcha version="2" size="compact"');
		});

		$(bcForm).find('fieldset:contains("{module_captchav2}") span').contents().filter(function() { return this.nodeType == 3 }).each(function(){
		    this.textContent = this.textContent.replace('module_captchav2','module_recaptcha version="2" size="compact"');
		});

		$(bcForm).find('fieldset:contains("{module_captcha}") span').contents().filter(function() { return this.nodeType == 3 }).each(function(){
		    this.textContent = this.textContent.replace('module_captcha','module_recaptcha version="2" size="compact"');
		});
